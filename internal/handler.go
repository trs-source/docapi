...
import (
	...
	files "github.com/swaggo/files/v2"
)
...
func Handle(endpointURL string) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		submatch := regMust.FindStringSubmatch(r.RequestURI)

		path := submatch[2]
		w.Header().Set("Content-Type", getContextType(path))
		...
		default:
			var err error
			r.URL, err = url.Parse(submatch[2])
			if err != nil {
				slog.Error("error when creating Swagger.", "error", err.Error())
				http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
				return
			}

			http.FileServer(http.FS(files.FS)).ServeHTTP(w, r)
		}
	}
}

func getContextType(path string) (ct string) {
